...
interface TestSpeakingSubmission {
  id: number;
  attemptId: number;
  userId: string;
  testId: number;
...
export function TestSpeakingGradingView({ submissionId }: { submissionId: number }) {
  const router = useRouter();
  const [submission, setSubmission] = useState<TestSpeakingSubmission | null>(null);
  ...
  const fetchSubmission = async () => {
    try {
      ...
      if (data.fluencyCoherenceScore !== null) {
        setFormData({
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      // Validate scores
      const scores = [
        formData.fluencyCoherenceScore,
        ...
        <div className="flex items-center gap-4">
          ...
          <div>
            <h1 className="text-3xl font-bold text-gray-900">
              Test Speaking Submission Review
            </h1>
            ...
            <CardContent>
              {submission.audioUrl ? (
                <audio controls className="w-full">
                  ...
                  <source src={submission.audioUrl} type="audio/mp4" />
          ...
          <form onSubmit={handleSubmit} className="space-y-6">
            <Card>
              ...
              <CardContent className="space-y-4">
                <div>
                  <Label htmlFor="fluencyCoherence">Fluency & Coherence (0-9)</Label>
                  <Input
                    id="fluencyCoherence"
                    type="number"
                    min="0"
                    ...
                    onChange={(e) =>
                      setFormData({ ...formData, fluencyCoherenceScore: e.target.value })
                    }
                    required
                  />
                </div>
                ...
                <div>
                  ...
                  <Input
                    ...
                    onChange={(e) =>
                      setFormData({ ...formData, pronunciationScore: e.target.value })
                    }
                    required
                  />
                </div>
