/**
 * Test Admission Test with Score 4.5-5.0
 * Creates an admission test attempt with band score 4.5-5.0
 * to test course recommendations for lower level students
 * Run with: npx tsx scripts/test-admission-score-4-5.ts
 */
...
async function testAdmissionScore45() {
  console.log("🚀 Testing Admission Test with Score 4.5-5.0\n");
  ...
  try {
    ...
    // Step 3: Submit answers - only 50% correct to get ~4.5-5.0 band score
    console.log("✏️  Step 3: Submitting answers (50% correct)...");
    let answeredCount = 0;
    let correctCount = 0;
    ...
    for (const section of admissionTest.sections) {
      for (let i = 0; i < section.questions.length; i++) {
        ...
        const isCorrect = i % 2 === 0; // Every other question is correct (50%)
    ...
    // Step 4: Complete test with 4.5-5.0 band score
    ...
    const [completedAttempt] = await db
      ...
      .set({
        ...
        score: scoreEarned,
        bandScore: 4.75,
        readingBandScore: 4.5,
        listeningBandScore: 5.0,
    ...
    console.log(`  - Overall Band Score: 4.75 (Lower level)`);
    console.log(`  - Reading Band: 4.5`);
    console.log(`  - Listening Band: 5.0`);
    console.log(`\n🔗 Result page: /admission-test/${admissionTest.id}/result/${attempt.id}`);
    console.log(`\n💡 This score should show beginner-level courses in recommendations`);
