CREATE TYPE "public"."type" AS ENUM('SELECT', 'ASSIST', 'GRAMMAR', 'VOCABULARY', 'LISTENING', 'READING');--> statement-breakpoint
...
ALTER TABLE "vocabulary_words" ADD CONSTRAINT "vocabulary_words_topic_id_vocabulary_topics_id_fk" FOREIGN KEY ("topic_id") REFERENCES "public"."vocabulary_topics"("id") ON DELETE cascade ON UPDATE no action;